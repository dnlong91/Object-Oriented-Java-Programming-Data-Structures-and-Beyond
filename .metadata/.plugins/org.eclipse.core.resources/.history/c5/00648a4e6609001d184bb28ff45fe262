 ...
 * mapping.
 * 
 * To accomplish this task, we break things into 3 steps:
 * 1. Setup map
	...
	private void shadeCountries() {
		for (Marker marker : countryMarkers) {
			...
			if (lifeExpByCountry.containsKey(countryId)) {
				...
				// [40, 90] is the range of life expectancy, [10, 255] is the range of the color we're gonna use
				// Here we translate the range of life expectancy to something to do with color by mapping those 2 ranges
				// The we need to type case to integer since RGB color code only deals with integers
				int colorLevel = (int) map(lifeExp, 40, 90, 10, 255);
				marker.setColor(color(255-colorLevel, 100, colorLevel));
			}
			else {
				marker.setColor(color(150,150,150)); // Default color
			}
		}
	}
